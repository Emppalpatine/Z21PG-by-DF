...
//        if (digitalRead(ShortExtPin) == HIGH)       //DFCH
      ...
      case csServiceMode:
          ...
          dcc.setpower(SERVICE); //already on!
            ...
            #else
              digitalWrite(ProgRelaisPin, HIGH);     //ProgTrack 
              digitalWrite(NPROG, LOW);         //<<DFCH>> NPROG high
            #endif
          #endif 
          #if defined(INA219)
...
//  if ((digitalRead(ShortExtPin) == LOW) && (digitalRead(GoExtPin) == BOOSTER_EXT_ON) && (Railpower != csShortCircuit)) {  //<<DFCH>>
    ShortTime++;
